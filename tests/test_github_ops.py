...
def github_error(status: int, message: str) -> GithubException:
    """Create a realistic PyGithub exception for mocked API failures."""
...
class FakeRepo:
    def __init__(
        ...
        *,
        ...
        self.existing_branch_ref = FakeGitRef("existing-branch-sha")
    ...
    def get_git_ref(self, ref: str):
        ...
        if ref == f"heads/{self.default_branch}":
        ...
        if ref.startswith("heads/remediation/"):
            ...
            raise github_error(404, "Reference not found")

        raise AssertionError(f"Unexpected Git reference requested: {ref}")
    ...
    def create_git_blob(self, content: str, encoding: str):
        ...
        return FakeBlob(f"blob-{len(self.created_blobs)}")
    ...
    def create_git_commit(self, message: str, tree, parents):
        ...
        return FakeCreatedCommit("new-commit-sha")
...
def test_creates_multiple_files_in_one_commit():
    ...
    result = delivery.create_remediation_pr(
        changed_files={
            "project/app.py": "print('updated app')\n",
            "project/routes.py": "print('updated routes')\n",
        },
        flag_name="OLD_FLAG",
    )

    assert result.url == "https://github.com/example/repo/pull/1"
    assert result.branch_name == "remediation/remove-old-flag"
    ...
    tree_elements = repo.created_tree_args["tree"]

    assert len(tree_elements) == 2
    assert tree_elements[0].path == "project/app.py"
    assert tree_elements[0].mode == "100644"
    assert tree_elements[0].type == "blob"
    assert tree_elements[0].sha == "blob-1"

    assert tree_elements[1].path == "project/routes.py"
    assert tree_elements[1].sha == "blob-2"

    assert repo.created_tree_args["base_tree"] is repo.base_tree

    assert repo.created_commit_args["message"] == (
        "chore: remove deprecated feature flag OLD_FLAG"
    )
    assert repo.created_commit_args["tree"].sha == "new-tree-sha"
    assert repo.created_commit_args["parents"] == [repo.base_commit]

    # The branch is created directly at the final commit.
    assert repo.created_ref_args == {
        "ref": "refs/heads/remediation/remove-old-flag",
        "sha": "new-commit-sha",
    }

    assert repo.created_pull_args["base"] == "main"
...
def test_uses_repository_default_branch():
    ...
    delivery.create_remediation_pr(
        changed_files={"app.py": "print('updated')\n"},
        flag_name="OLD_FLAG",
    )

    assert "heads/develop" in repo.get_git_ref_calls
    assert repo.created_pull_args["base"] == "develop"
...
def test_explicit_base_branch_overrides_default():
    ...
    assert "heads/release" in repo.get_git_ref_calls
    assert repo.created_pull_args["base"] == "release"
...
def test_normalizes_windows_paths_before_creating_tree():
    ...
    delivery.create_remediation_pr(
        changed_files={
            r"project\core\mutator.py": "print('updated')\n",
        },
        flag_name="OLD_FLAG",
    )

    tree_element = repo.created_tree_args["tree"][0]

    assert tree_element.path == "project/core/mutator.py"
...
def test_rejects_invalid_paths_before_any_remote_write(bad_path):
    repo = FakeRepo()
    delivery = GitOpsDelivery(repo=repo)

    with pytest.raises((TypeError, ValueError)):
        delivery.create_remediation_pr(
            changed_files={bad_path: "content"},
            flag_name="OLD_FLAG",
        )
...
def test_returns_existing_open_pr_without_writes():
    ...
    assert result.commit_sha == "existing-pr-commit"
...
def test_existing_pr_query_uses_owner_branch_filter():
    ...
    assert repo.get_pulls_calls[0] == {
        "state": "open",
        "base": "main",
        "head": "example:remediation/remove-old-flag",
    }
...
def test_rejects_existing_branch_without_open_pr():
    ...
    with pytest.raises(
        DeliveryConflictError,
        match="already exists but has no open pull request",
    ):
        delivery.create_remediation_pr(
            changed_files={"app.py": "print('updated')\n"},
            flag_name="OLD_FLAG",
...
def test_branch_creation_422_rechecks_for_existing_pr():
    ...
    assert result.url == "https://github.com/example/repo/pull/9"
    assert result.reused_existing_pr is True
...
def test_pull_creation_422_rechecks_for_existing_pr():
    ...
    assert result.url == "https://github.com/example/repo/pull/10"
    assert result.reused_existing_pr is True
...
def test_pull_creation_422_without_pr_raises_conflict():
    ...
    with pytest.raises(
        DeliveryConflictError,
        match="GitHub rejected pull request creation",
    ):
        delivery.create_remediation_pr(
            changed_files={"app.py": "print('updated')\n"},
            flag_name="OLD_FLAG",
...
def test_rejects_empty_changed_files():
    delivery = GitOpsDelivery(repo=FakeRepo())
...
def test_branch_name_is_deterministic_and_sanitized():
    ...
    ) == "remediation/remove-enable-new-dashboard"


def test_branch_name_has_bounded_length():
    delivery = GitOpsDelivery(repo=FakeRepo())

    branch_name = delivery.build_branch_name("A" * 300)

    assert branch_name.startswith("remediation/remove-")
    assert len(branch_name) <= len("remediation/remove-") + 80